...
The repository uses private vulnerability reporting, dependency alerts/security updates, secret scanning with push protection, immutable GitHub Action pins, a required read-only quality gate, explicit CodeQL scanning for repository-owned JavaScript/TypeScript, Python, and GitHub Actions, checksum-verified ShellCheck scanning for tracked shell programs, checksum-verified actionlint semantic validation for tracked workflows, public-safety checks, exact upstream source guards, and published image provenance/attestation.
